...
public class Main {
	static int N, M;
	static long[] numList, minSegTree, maxSegTree;
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int i = 0; i < M; i++) {
			...
			sb.append(findMin(0, low, up, 0, N)).append(' ').append(findMax(0, low, up, 0, N)).append('\n');
		}
		System.out.println(sb);
	}

	private static long findMax(int curIdx, int low, int up, int start, int end) {
		int mid = (start+end)>>1;
		if (low <= start && up >= end) {// 주어진 범위를 포함
			return maxSegTree[curIdx];
		} else if (low > start && up >= end) {
			if(mid<=low) {
				return findMax(curIdx*2+2, low, up, mid, end);
			}
			else {
				return Math.max(findMax(curIdx*2+2, low, up, mid, end), findMax(curIdx*2+1, low, up, start, mid));
			}
		} else if (low <= start && up < end) {
			if(mid>=up) {
				return findMax(curIdx*2+1, low, up, start, mid);
			}
			else {
				return Math.max(findMax(curIdx*2+2, low, up, mid, end), findMax(curIdx*2+1, low, up, start, mid));
			}
	...
	private static long setMax(int curIdx, int start, int end) {
		if (start == end - 1)
			return maxSegTree[curIdx] = numList[start];
		int mid = (start + end) >> 1;
		return maxSegTree[curIdx] = Math.max(setMax(curIdx * 2 + 1, start, mid), setMax(curIdx * 2 + 2, mid, end));
	}
